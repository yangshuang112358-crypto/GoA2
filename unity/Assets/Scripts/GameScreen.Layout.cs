        ...
        private void BuildHand(VisualElement parent, GameView view)
        {
            ...
            foreach (var instance in view.OwnCards)
            {
                ...
                var tile = Button("", () =>
                {
                    ...
                    else if (view.ForcedDiscardCards.Contains(card.Id) || view.OptionalDiscardCards.Contains(card.Id) || view.CardSwapOptions.Contains(card.Id)) { discardCardId=card.Id;declineRetaliationPending=false;showDebug=false;Render(); }
