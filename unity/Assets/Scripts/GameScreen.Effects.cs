    ...
    public sealed partial class GameScreen
    {
        ...
        private void RenderActiveEffects(VisualElement parent,GameView view)
        {
            ...
            foreach(var effect in view.Effects.OrderBy(e => e.CreationOrder))
            {
                ...
                {
                    string meaning=effect.Kind==EffectKind.FriendlyBasicMinionsRanged ? "本队基础攻击时，范围内友方小兵视为远程" : effect.Kind==EffectKind.NonAdjacentRangedImmunity ? "自身免疫非相邻英雄的远程攻击" : effect.Kind==EffectKind.MovementBoundary ? "敌方移动不能跨越范围边界" : effect.AreaKind==EffectAreaKind.Adjacent ? "相邻敌方英雄不能执行技能" : "范围内敌方英雄不能执行技能";
