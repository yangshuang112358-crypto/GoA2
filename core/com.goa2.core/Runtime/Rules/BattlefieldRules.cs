...
namespace Goa2.Rules
{
    public sealed partial class GameRules
    {
        ...
        private static void RemoveMinion(ContentCatalog catalog, GameState state, Command command, string unitId,
            string source, int? rewardSeat = null, bool bypassProtection = false, bool finishActionOnResume = false)
        {
            ...
            if (unit.Kind == "heavy") AdvanceFrontline(catalog, state, command, unit.Team, source, finishActionOnResume);
            else if (finishActionOnResume) FinishAction(catalog, state, command);
        }
        private static void AdvanceFrontline(ContentCatalog catalog, GameState state, Command command, Team defeated, string source, bool finishActionOnResume)
        {
            ...
            var resume = new FrontlineTransition
            {
                ...
                Source = source, FinishActionOnResume = finishActionOnResume
            };
        ...
        private static void DeclareVictory(GameState state, Command command, Team winner, string reason)
        {
            ...
            state.ActiveSeat = null; state.Pending = null; state.Frontline = null;
