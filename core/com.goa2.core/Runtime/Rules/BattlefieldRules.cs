...
namespace Goa2.Rules
{
    public sealed partial class GameRules
    {
        ...
        private static void RemoveMinion(ContentCatalog catalog, GameState state, Command command, string unitId,
            string source, int? rewardSeat = null, bool bypassProtection = false, bool finishActionOnResume = false, bool resumeCardExecution = false, bool resumeRoundEnd = false, bool preventionResolved = false, bool resumeActionMinionBattle = false)
        {
            ...
            if (unit.Kind == "heavy") AdvanceFrontline(catalog, state, command, unit.Team, source, finishActionOnResume, resumeCardExecution, resumeRoundEnd, resumeActionMinionBattle);
            else if (finishActionOnResume) FinishAction(catalog, state, command);
        }
        private static void AdvanceFrontline(ContentCatalog catalog, GameState state, Command command, Team defeated, string source, bool finishActionOnResume, bool resumeCardExecution, bool resumeRoundEnd, bool resumeActionMinionBattle)
        {
            ...
            var resume = new FrontlineTransition
            {
                ...
                Source = source, FinishActionOnResume = finishActionOnResume, ResumeCardExecution = resumeCardExecution, ResumeRoundEnd = resumeRoundEnd, ResumeActionMinionBattle = resumeActionMinionBattle
            };
        ...
        private static void ContinueFrontline(ContentCatalog catalog, GameState state, Command command)
        {
            ...
            if (progress.Remaining.Count == 0)
            {
                ...
                else if (progress.ResumeActionMinionBattle) ContinueActionMinionBattle(catalog, state, command);
                return;
            }
