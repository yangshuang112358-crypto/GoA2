...
namespace Goa2.Rules.Cards
{
    ...
    internal static class CardPrograms
    {
        ...
        private static readonly Dictionary<string,(string text,int minimumEngine,PrimaryProgram program)> Attacks = new Dictionary<string,(string,int,PrimaryProgram)>
        {
            ...
            ["arien-05-巨浪滔天"] = ("选择攻击距离内的一个单位为目标。攻击后：将目标移动最多1格。将最多一个与你相邻的标志物或敌方单位推动1格。",77,new PrimaryProgram("ranged_attack_move_target_then_optional_push",1,textMoveDistance:1,textPushDistance:1,pushTarget:PushTargetKind.EnemyUnit,optionalPushTarget:true,fixedPushDistance:true,instructions:new[]{InstructionKind.ChooseAttackTarget,InstructionKind.Attack,InstructionKind.OptionalTargetUnitMove,InstructionKind.ChooseUnitPush,InstructionKind.End})),
            ["arien-03-潮起"] = ("选择攻击距离内的一个单位为目标。攻击后：将最多一个与你相邻的标志物或敌方单位推动1格。",76,new PrimaryProgram("ranged_attack_then_optional_adjacent_fixed_push",1,textPushDistance:1,pushTarget:PushTargetKind.EnemyUnit,optionalPushTarget:true,fixedPushDistance:true,instructions:new[]{InstructionKind.ChooseAttackTarget,InstructionKind.Attack,InstructionKind.ChooseUnitPush,InstructionKind.End})),
            ...
            ["wasp-05-电能爆炸"] = ("选择与你相邻的一个单位为目标。攻击前：技能范围内最多一名敌方英雄（除攻击目标外）丢弃一张卡牌，否则被击败。",71,new PrimaryProgram("adjacent_attack_optional_other_hero_pay_or_defeat",1,adjacent:true,heroTarget:HeroTargetKind.OtherEnemyInSkillRange,attackSubtype:"范围",instructions:new[]{InstructionKind.ChooseAttackTarget,InstructionKind.OptionalOtherHeroDiscardOrDefeat,InstructionKind.Attack,InstructionKind.End})),
        ...
        private static readonly Dictionary<string,(string text, int minimumEngine, string? subtype, PrimaryProgram program)> Skills = new Dictionary<string,(string, int, string?, PrimaryProgram)>
        {
            ...
            ["wasp-09-意念操控"] = ("将攻击距离内与你不在同一直线上的一个单位或标志物放置到与你相邻的格子。",78,"远程",new PrimaryProgram("place_nonlinear_unit_adjacent_to_self",0,instructions:new[]{InstructionKind.ChooseUnitPlacement,InstructionKind.End})),
            ...
            ["arien-09-魔法水流"] = ("将你放置到攻击距离内没有出生点，且不与空的出生点相邻的格子内。",69,"远程",new PrimaryProgram("self_place_away_from_empty_spawns",0,placementTarget:PlacementTargetKind.EmptyNoSpawnAwayFromEmptySpawns,instructions:new[]{InstructionKind.ChooseSelfPlacement,InstructionKind.End})),
            ["arien-07-潮水"] = ("将你放置到攻击距离内没有出生点，且不与空的出生点相邻的格子内。",68,"远程",new PrimaryProgram("self_place_away_from_empty_spawns",0,placementTarget:PlacementTargetKind.EmptyNoSpawnAwayFromEmptySpawns,instructions:new[]{InstructionKind.ChooseSelfPlacement,InstructionKind.End})),
            ...
            ["brogan-16-野蛮飞踢"] = ("移动最多1格，然后选择一项：移除与你相邻的一个标志物；或将与你相邻的一个敌方单位推动最多2格。",74,null,new PrimaryProgram("move_one_then_push_enemy_unit",0,textMoveDistance:1,textPushDistance:2,pushTarget:PushTargetKind.EnemyUnit,instructions:new[]{InstructionKind.OptionalTextMove,InstructionKind.ChooseUnitPush,InstructionKind.End})),
            ["brogan-14-碾压重拳"] = ("移动最多1格，然后选择一项：移除与你相邻的一个标志物；或将与你相邻的一个敌方小兵推动最多2格。",73,null,new PrimaryProgram("move_one_then_push_enemy_minion",0,textMoveDistance:1,textPushDistance:2,instructions:new[]{InstructionKind.OptionalTextMove,InstructionKind.ChooseUnitPush,InstructionKind.End})),
            ...
            ["shargatha-06-海妖之歌"] = ("选择攻击距离内与你不相邻的、最近的、非免疫的敌方单位为目标；沿最短有效路径将该单位向你移动最多2格。可重复一次。",67,"远程",new PrimaryProgram("nearest_enemy_approach_twice",0,textMoveDistance:2,instructions:new[]{InstructionKind.ChooseNearestApproachTarget,InstructionKind.OptionalApproachMove,InstructionKind.OptionalRepeatApproach,InstructionKind.End})),
