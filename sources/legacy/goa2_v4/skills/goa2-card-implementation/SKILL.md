...
description: Analyze, specify, implement, test, and review Guards of Atlantis II (GoA2) card effects in the GoA2 v4 project. Use when Codex is asked to interpret a card, resolve card-rule ambiguity, design or update an effect schema, implement card behavior or engine primitives, create comprehensive card tests, audit implementation status, or generate a small Codex task for GoA2 card work.
---
...
Locate the GoA2 v4 repository, then read:
...
3. `GOA2_RULEBOOK.md`
4. Tested v4 behavior
5. v2/v3 behavior as non-authoritative migration evidence
...
Planning and lifecycle baseline:

- each round contains exactly four turns;
- players may reselect until they explicitly confirm;
- confirmation locks that player's selection;
- the fourth confirmation automatically reveals all four cards;
- there is no player-facing manual reveal/sync button;
- the initial decision-coin color is randomly red or blue;
- frontline victory defaults to the third push, visually taking the center of
  a five-circle track.
...
At v4 initialization, every card starts at `data_only` with the user-facing
label `未实装未测试`. Never migrate a v2/v3 implementation status into v4.
