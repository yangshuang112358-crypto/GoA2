...
GoA2 电子版重启工程。[BATCH-02](docs/planning/BATCH-02.md)正在持续开发：Unity与Windows程序已可运行，测试模式、四边折叠界面和地图缩放已接入。首批两张主要攻击与三张主要防御可操作，支持英雄击败/复活、小兵击败/推进/出生及胜负。共享核心Unity104项通过，.NET在补充三项前101项通过；基础界面两个分辨率各19项、战场9项、战斗10项真实输入检查通过；六份场景可无窗口运行或可见播放。正式数据为6英雄、108牌、254格。其余牌效、持续效果、完整轮末/升级与联网仍未实现。
...
按1/2/3/4或点击左侧立即切换角色。地图支持滚轮缩放、中/右键拖动、Home全图；四周区域可折叠。地图点击只产生预选，确认后才提交规则命令。拔枪/快速突刺可开始主要攻击，防御者切至自己席位用手牌响应；复活先选出生点再继续原牌。尚未支持的主要文字和轮末结算会明确显示，不能当作完整对局。
