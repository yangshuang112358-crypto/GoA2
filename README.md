...
GoA2 电子版重启工程。[BATCH-02](docs/planning/BATCH-02.md)正在持续开发：Unity与Windows程序已可运行，测试模式、四边折叠界面和地图缩放已接入。小兵击败、推进、占位出生选择及胜负核心可由调试入口操作。同源纯C#核心69项测试在.NET通过（Unity最近68项），两个分辨率各19项基础界面检查、9项战场界面检查通过；五份场景可无窗口运行或可见播放。正式数据为6英雄、108牌、254格。主要牌效、完整轮末/升级与联网仍未实现。
