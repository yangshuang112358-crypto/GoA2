...
当前引擎8接入21张牌：11张主要攻击、8张防御及静电封锁/打断施法，涵盖条件距离、攻击前可选弃牌、反制强制弃牌、空手反击和部分持续效果。常规击败/复活、兵线推进、轮末回收/移兵、连续升级、补偿与下一轮均可操作。剩余87张牌、完整触发联动、紫卡文字和联网仍待开发，详见[剩余卡牌路线](docs/planning/剩余卡牌开发路线.md)与[下一批计划](docs/planning/下一批开发计划.md)。

本批共享核心Unity394项通过；14模块实际键鼠在1280×800和1152×768各238项通过。29份场景可无窗口运行或可见播放，四组较长模拟共2321条命令完成一致重放，其中两组自然获胜。本机.NET测试宿主受Windows应用控制限制，结果与Unity同源测试分别记录；不把历史158项当成本次完整.NET通过。

立即体验：运行 artifacts/player/Goa2V1.exe。操作与开发入口见[启动与操作指南](docs/development/启动与操作指南.md)，结果和程序包见[本批验收](docs/verification/BATCH-02验收.md)，过程见[本批记录](docs/verification/BATCH-02工作记录.md)。

本机完整交付入口：artifacts/delivery/Goa2V1-BATCH02-20260910/开始使用.md，包含Windows包、源码与规则资料、离线浏览器和带哈希的验收证据。后续从源码重建时按下方开发运行步骤生成本机产物。
