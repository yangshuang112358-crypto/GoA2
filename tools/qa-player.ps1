param(
    [ValidateSet("Capture","Click","Scroll")][string]$Action = "Capture",
    [int]$X = 0, [int]$Y = 0,
    [string]$Caption,
...
if ($Action -ne "Capture") {
    if ($Caption) {
        ...
        do {
            ...
            $goaMatches = @($goaLayout.Buttons | Where-Object { $_.Enabled -and $_.Text -match $Caption })
            if ($goaMatches.Count -eq 1) { break }
            [System.Threading.Thread]::Sleep(100)
        } while ([DateTime]::UtcNow -lt $goaDeadline)
        if ($goaMatches.Count -ne 1) { throw "Caption must match exactly one enabled button; matched $($goaMatches.Count)." }
