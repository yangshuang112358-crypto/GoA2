...
(async()=>{
 ...
 try{
  ...
  await page.selectOption('#hero-filter','');await page.selectOption('#status-filter','implemented');check(await page.locator('.card').count()===21,'Implementation status is separate from the full catalog');
