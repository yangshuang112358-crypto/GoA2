...
- engine9-riposte-hand.json、engine9-riposte-empty-defense.json：在引擎10修改前由真实引擎9 Player分别捕获有牌强制弃牌与空手待防御窗口。
...
manifest.json登记八份历史存档及七份对应输入。资料校验直接核对原字节SHA-256、引擎版本、命令数量、来源说明与完整登记范围；JSON语义不变的换行或空白改动也会失败。新增真实旧档时同时登记清单及取证说明，不能通过更新摘要来掩盖旧档被改写。test_fixtures.py在独立临时副本验证九项完整性及负例，不改原始捕获。
